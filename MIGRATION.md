# Migration Guide - YellowSphere v1.6.6

This guide covers upgrading to the current v1.6.6 build and migrating `.arc`
seed backups. The canonical implementation is `YellowSphere.html` v1.6.6.

## Upgrade Checklist

1. Transfer the full project folder or verified release artifact.
2. Verify hashes before entering real seed material.
3. Keep `YellowSphere.html`, promoted HTML/assets, Python files,
   Tauri files, docs, and release artifacts from the same build set.
4. Test with a known mnemonic before operational use.
5. Re-export older `.arc` files with v1.6.6 when practical.
6. Back up any new keyfiles created during migration.

Do not combine a v1.6.6 root HTML file with older packaged assets or wrappers
unless you are intentionally testing a compatibility boundary.

## Feature Migration Notes

The v1.6.6 HTML adds or formalizes:
...
- `lts/YellowSphere_v1.6.2_LTS.html`
- `lts/YellowSphere_v1.6.0_LTS.html`
...
| --- | --- | --- |
| `yellowsphere-encrypted-seed-v3` inside `YELLOWSPHERE-ARC-V2` armor | Current v1.6.6 protected export | AES-256-GCM, authenticated metadata, 32-byte salt, 12-byte nonce, fixed 512-byte padded plaintext |
| `yellowsphere-encrypted-seed-v2` inside `YELLOWSPHERE-ARC-V2` armor | Legacy protected import | PBKDF2-HMAC-SHA512, HMAC-SHA512 stream/MAC, 32-byte salt, 24-byte nonce |
| `YELLOWSPHERE-ARC-V2` armored plain | Current unprotected export option | Plain mnemonic JSON inside armor; not encrypted |
| Raw JSON `yellowsphere-encrypted-seed-v2` | Import supported | Earlier V2 storage without armor |
| `yellowsphere-encrypted-seed-python-v1` | Import supported | Legacy Python PBKDF2-SHA256/HMAC stream family |
| Browser `yellowsphere-encrypted-seed-v1` | Browser import supported | Legacy PBKDF2-SHA256/AES-GCM family |
| Headerless legacy JSON | Import supported where structurally recognized | Old compatibility path |

In v1.6.6, the armor header remains `YELLOWSPHERE-ARC-V2`, but newly
protected bundles carry internal version 3 and format
`yellowsphere-encrypted-seed-v3`. Do not identify the cryptographic format from the
armor line alone; decode the JSON bundle and inspect its `version` and `format`.

New v1.6.6 password-protected keyfiles carry `version: 2` while retaining the
`YELLOWSPHERE-KEYFILE-ENC` magic and `yellowsphere-keyfile-enc-v1` format string.
...
1. Start v1.6.6 on a trusted offline machine.
...
Re-exporting with v1.6.6 creates YellowSphere ARC V3 and, in combined
...
- `YellowSphere_Encrypted_Seed.arc`
- `YellowSphere_Keyfile.key`

Keyfile + Password mode requires:

- `YellowSphere_Encrypted_Seed.arc`
- `YellowSphere_Keyfile.enc.key`
...
- you want v1.6.6 QR or PDF behavior
...
```powershell
Get-ChildItem -Recurse -File YellowSphere.html,lts,YellowSphere.py,src,vendor,scripts,src-tauri,docs |
...
```bash
find YellowSphere.html lts YellowSphere.py src vendor scripts src-tauri docs \
...
- rebuild or inspect `docs/YellowSphere_Manual.pdf` when documentation changes
