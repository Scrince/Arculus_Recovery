...
def find_html_app() -> Path:
    candidates = [
        Path.cwd() / "Arculus_Recovery.html",
        Path(__file__).resolve().parents[2] / "Arculus_Recovery.html",
        Path(sys.argv[0]).resolve().parent / "Arculus_Recovery.html",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    packaged = resources.files("arculus_recovery.assets").joinpath("Arculus_Recovery.html")
    if packaged.is_file():
        context = resources.as_file(packaged)
        asset_path = context.__enter__()
        _PACKAGED_ASSET_CONTEXTS.append(context)
        return asset_path
    raise FileNotFoundError("Arculus_Recovery.html was not found next to the launcher, in the current directory, or in packaged assets.")
...
def find_vendored_jspdf() -> Optional[Path]:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    packaged = resources.files("arculus_recovery.assets").joinpath("jspdf.umd.min.js")
    if packaged.is_file():
        context = resources.as_file(packaged)
        asset_path = context.__enter__()
        _PACKAGED_ASSET_CONTEXTS.append(context)
...
def launch_gui() -> None:
    ...
    if jspdf_path:
        ...
        view.page().profile()._arculus_offline_interceptor = interceptor
    view.setUrl(QUrl.fromLocalFile(str(html_path)))
    window.setCentralWidget(view)

    def choose_download_path(download) -> None:
        suggested = download.suggestedFileName() or "Arculus_Recovery_export"
